...
{
    public sealed partial class TextEncodingUI : UserControl, ICryptography
    {
        ...
        public async void SaveSourceAsFile() => await PickAndSaveFile(CurrentEncoding.GetBytes(TxtSource.Text), new Dictionary<string, string>() { { "文本文件", ".txt" } }, true);

        public async void SaveResultAsFile() => await PickAndSaveFile(CurrentEncoding.GetBytes(TxtResult.Text), new Dictionary<string, string>() { { "文本文件", ".txt" } }, true);
        ...
        string ICryptography.Encoding => (cbbSaveEncoding.SelectedItem as ComboBoxItem).Content as string;
